...
export default function signup({ userIdCookie }) {
  ...
  const handleSubmit = async (event) => {
    ...
    const regExp = /^2451-\d{2}-\d{3}-\d{3}$/; // regular expression pattern for nntttnnnnn format
    if (regExp.test(regNumber)) {
      ...
      setMessage({
        errorMsg: "Registration Number is not valid",
        successMsg: "",
      });
    }
  };
...

          {
            ...
            step === 2 && (
              <form onSubmit={handleSubmit}>
                ...
                <div>
                  <label className="block mb-2 text-sm font-medium text-gray-700">
                    Enter your Registration Number(eg: 2451-19-737-123)
                  </label>
                  <input
                    type="text"
                    id="regNumber"
